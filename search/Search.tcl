...
define_cmd_args "report_checks" \
  {[-from from_list|-rise_from from_list|-fall_from from_list]\
     ...
     [-fields capacitance|slew|input_pin|net|src_attr|netlist_src]\
     [-digits digits]\
     [-no_line_splits]\
     [> filename] [>> filename]}
...
proc parse_report_path_options { cmd args_var default_format
  ...
  if { [info exists path_options(-fields)] } {
    ...
    set report_netlist_src [expr [lsearch $fields "netlist_src*"] != -1]
  } else {
    ...
    set report_netlist_src 0
  }
  set_report_path_fields $report_input_pin $report_net \
    $report_cap $report_slew $report_fanout $report_src_attr $report_netlist_src
