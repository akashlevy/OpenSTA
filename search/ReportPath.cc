...
ReportPath::makeFields()
{
  ...
  field_description_ = makeField("description", "Description", 36, 
				 true, nullptr, true);
  field_src_attr_ = makeField("src_attr", "Src Attr", 30,
			      true, nullptr, true);
  field_netlist_src_ = makeField("netlist_src", "Netlist Src", 30,
				 true, nullptr, true);
}
			    ...
			    bool report_netlist_src)
{
  ...
  field_netlist_src_->setEnabled(report_netlist_src);
  // for debug
			   ...
			   const char *line_case)
{
  ...
  // TODO: also get src_attr/netlist_src for nets.
  string src_attr = "";
  string netlist_src = "";
  if (inst) {
    src_attr = network_->getAttribute(inst, "src");
    netlist_src = "TODO"; // TODO: get netlist linefile string.
  }
			...
			float time_offset)
{
  ...
  for (size_t i = path_first_index; i <= path_last_index; i++) {
	...
	// Don't show capacitance field for input pins. FIXME: but we can???
	if (is_driver && field_capacitance_->enabled())
          cap = graph_delay_calc_->loadCap(pin, rf, dcalc_ap);
	// Don't show fanout field for input pins. FIXME: but we can???
	...
	if (report_net_ && is_driver) {
	  reportLine(what.c_str(), cap, slew, fanout,
		     ...
		     src_attr, netlist_src, line_case);
	  string what2;
	  if (network_->isTopLevelPort(pin)) {
	    const char *pin_name = cmd_network_->pathName(pin);
	  ...
	  reportLine(what2.c_str(), field_blank_, field_blank_, field_blank_,
		     field_blank_, field_blank_, false, min_max,
                     nullptr, src_attr, netlist_src, line_case);
	}
	else {
	  reportLine(what.c_str(), cap, slew, fanout,
		       ...
		       const char *line_case)
{
  ...
  while (field_iter.hasNext()) {
	...
	if (src_attr != "")
      ...
      else if (field == field_netlist_src_) {
	if (netlist_src != "")
	  reportField(netlist_src.c_str(), field, line);
	else
	  reportFieldBlank(field, line);
      }
